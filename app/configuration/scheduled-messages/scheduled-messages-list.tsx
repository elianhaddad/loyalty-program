...
export default function ScheduledMessagesList({ initialMessages }: ScheduledMessagesListProps) {
  ...
  const handleSearch = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (value === "") {
      ...
      const filtered = initialMessages.filter(
        (message) => message.name.toLowerCase().includes(value) || message.message.toLowerCase().includes(value),
      )
  ...
  const handleToggleActive = async (id: string, currentActive: boolean) => {
    try {
      ...
      // Update local state
      setMessages((prev) => prev.map((m) => (m._id === id ? { ...m, active: !currentActive } : m)))
                  ...
                  <FormControlLabel
                    control={
                      <Switch
                        ...
                        onChange={() => handleToggleActive(message._id, message.active)}
                        color="primary"
                      />
                    }
                    label=""
