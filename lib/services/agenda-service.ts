...
export async function getAgenda() {
  ...
  agenda = new Agenda({
    db: {
      address:
        process.env.MONGODB_URI || "mongodb://localhost:27017/points-system",
  ...
  agenda.define("send scheduled message", async (job) => {
    ...
    try {
      ...
      console.log(
        `Sent scheduled message "${message.name}" to ${clients.length} clients`
      );
    } catch (error) {
...
export async function scheduleRecurringMessage(
  messageId: string,
  pattern: string,
  days: string[]
) {
